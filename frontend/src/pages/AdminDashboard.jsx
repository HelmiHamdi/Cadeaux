...
const AdminDashboard = () => {
  ...
  const [selectedGiftForCodes, setSelectedGiftForCodes] = useState(null);
  ...
  const handleManageCodes = (giftId, giftTitle) => {
    setSelectedGiftForCodes({ id: giftId, title: giftTitle });
  ...
  const handleSubmitNewGifts = async () => {
    ...
    try {
      ...
      setNewGifts(
        newGifts.map(() => ({ title: "", description: "", image: "" }))
      );
      fetchDashboardData();
      setActiveTab("dashboard");
    } catch (error) {
      setError(
        error.response?.data?.message || "Erreur lors de l'ajout des cadeaux"
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleManualDraw = async () => {
    ...
    try {
      ...
      await Window.success(
        "Tirage effectué avec succès! Les résultats ont été générés.",
        "Tirage Réussi"
      );
  ...
  return (
    <div className="min-h-screen bg-gray-100">
          ...
          <div className="flex justify-between items-center py-4">
            ...
            <div className="flex gap-4">
              <button
                ...
                className="bg-purple-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-purple-700 transition-colors popup-button"
              >
                <Trophy size={20} />
        ...
        {activeTab === "dashboard" && dashboardData && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
              {[
                {
                  label: "Cadeaux Actifs",
                  value: dashboardData.stats.activeGifts,
                  icon: Gift,
                  color: "blue",
                },
                {
                ...
                },
                {
                  ...
                  value: dashboardData.stats.totalDraws,
                  icon: Trophy,
                  color: "purple",
                },
                {
                  label: "Total Cadeaux",
                  value: dashboardData.stats.totalGifts,
                  icon: BarChart3,
                  color: "orange",
                },
                {
                  label: "Participants (7j)",
                  value: dashboardData.stats.weeklyParticipants,
                  icon: TrendingUp,
                  color: "red",
                },
              ].map((stat, index) => (
                <div
                  key={index}
                  className="bg-white rounded-lg shadow p-6 transform hover:scale-105 transition-transform duration-200"
                >
            ...
            <div className="bg-white rounded-lg shadow">
              <div className="px-6 py-4 border-b">
                <h2 className="text-lg font-semibold">
                  Cadeaux Actifs ({dashboardData.activeGifts.length})
                </h2>
                <p className="text-sm text-gray-600">Gérez vos cadeaux</p>
              </div>
              <div className="p-6">
                {dashboardData.activeGifts.length > 0 ? (
                        ...
                        <div className="relative">
                          ...
                          <div className="relative h-48 bg-gray-100 overflow-hidden">
                            <img
                              src={
                                gift.image ||
                                "https://via.placeholder.com/300x200?text=Image+Cadeau"
                              }
                              alt={gift.title}
                            ...
                            <div className="absolute top-3 right-3 flex gap-2">
                              <button
                                onClick={() =>
                                  handleManageCodes(gift._id, gift.title)
                                }
                                className="p-2 bg-white/90 backdrop-blur-sm text-purple-600 rounded-lg hover:bg-white hover:scale-110 transition-all duration-200 shadow-lg border border-white/20 popup-button"
                              ...
                              <button
                                onClick={() =>
                                  handleViewParticipants(gift._id, gift.title)
                                }
                              ...
                              <button
                                onClick={() =>
                                  handleDeleteGift(gift._id, gift.title)
                                }
                  ...
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-400 text-sm mt-2">
                      Ajoutez de nouveaux cadeaux via l'onglet "Nouveaux
                      cadeaux"
                    </p>
                  </div>
                )}
            ...
            {dashboardData.recentParticipants &&
          ...
          <div className="bg-white rounded-lg shadow">
            ...
            <div className="p-6 space-y-6">
              {newGifts.map((gift, index) => (
                <div
                  key={index}
                  className="border rounded-lg p-4 space-y-4 hover:border-blue-300 transition-colors duration-200"
                >
                  <div className="flex justify-between items-center">
                    ...
                    {newGifts.length > 1 && (
                      <button
                        onClick={() => {
                          const updatedGifts = newGifts.filter(
                            (_, i) => i !== index
                          );
                          setNewGifts(updatedGifts);
                        }}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        value={gift.image}
                        onChange={(e) =>
                          handleGiftChange(index, "image", e.target.value)
                        }
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                ...
                >
                  {loading ? (
                    ...
                    <>
                      <Plus size={20} />
                      Ajouter les{" "}
                      {newGifts.filter((g) => g.title.trim()).length} cadeaux
                      valides
                    </>
                  )}
                </button>
      ...
      <CodeManagement
        ...
        giftId={selectedGiftForCodes?.id}
        giftTitle={selectedGiftForCodes?.title}
      />
        <SelectWinnersModal
        isOpen={showSelectWinners}
