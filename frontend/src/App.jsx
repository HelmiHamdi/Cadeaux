import React, { useState, useEffect } from 'react';
import Header from './components/Header';
import Home from './pages/Home';
import About from './pages/About';
import Admin from './pages/Admin';

function App() {
  const [isLaunched, setIsLaunched] = useState(false);
  const [timeLeft, setTimeLeft] = useState({});
  const [loading, setLoading] = useState(true);

  // Date de lancement : 28 novembre 2025 à 20h00 temps Tunisien
  const launchDate = new Date('2025-11-28T20:00:00+01:00'); // UTC+1 pour la Tunisie

  // Vérifier si nous sommes sur la route admin
  const isAdminPage = window.location.pathname === '/admin';

  useEffect(() => {
    checkLaunchStatus();
    const timer = setInterval(checkLaunchStatus, 1000);
    return () => clearInterval(timer);
  }, );

  const checkLaunchStatus = () => {
    const now = new Date();
    const difference = launchDate - now;
    
    if (difference <= 0) {
      setIsLaunched(true);
      setLoading(false);
      return;
    }

    setIsLaunched(false);
    setLoading(false);
    
    // Calcul du temps restant
    const days = Math.floor(difference / (1000 * 60 * 60 * 24));
    const hours = Math.floor((difference % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
    const minutes = Math.floor((difference % (1000 * 60 * 60)) / (1000 * 60));
    const seconds = Math.floor((difference % (1000 * 60)) / 1000);

    setTimeLeft({
      days: days.toString().padStart(2, '0'),
      hours: hours.toString().padStart(2, '0'),
      minutes: minutes.toString().padStart(2, '0'),
      seconds: seconds.toString().padStart(2, '0')
    });
  };

  // Écran de compte à rebours avant le lancement (pour toutes les pages sauf si lancé)
  if (!isLaunched) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-primary via-secondary to-accent flex items-center justify-center p-4">
        <div className="text-center text-white max-w-4xl mx-auto">
          {/* Logo ou icône principale */}
          <div className="mb-8 animate-bounce">
            <div className="text-8xl mb-4">🎁</div>
          </div>
          
          {/* Titre principal */}
          <h1 className="text-4xl md:text-6xl font-bold mb-6 animate-pulse">
            Ça arrive bientôt !
          </h1>
          
          {/* Sous-titre */}
          <p className="text-xl md:text-2xl mb-12 opacity-90">
            Notre grand jeux de cadeaux débute le<br />
            <strong>28 Novembre 2025 à 20h00</strong>
          </p>
          
          {/* Compte à rebours */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 max-w-2xl mx-auto mb-12">
            <div className="bg-white bg-opacity-20 rounded-2xl p-6 backdrop-blur-sm border border-white border-opacity-30">
              <div className="text-3xl md:text-5xl font-bold mb-2 text-accent">
                {timeLeft.days}
              </div>
              <div className="text-sm md:text-base opacity-80">Jours</div>
            </div>
            
            <div className="bg-white bg-opacity-20 rounded-2xl p-6 backdrop-blur-sm border border-white border-opacity-30">
              <div className="text-3xl md:text-5xl font-bold mb-2 text-accent">
                {timeLeft.hours}
              </div>
              <div className="text-sm md:text-base opacity-80">Heures</div>
            </div>
            
            <div className="bg-white bg-opacity-20 rounded-2xl p-6 backdrop-blur-sm border border-white border-opacity-30">
              <div className="text-3xl md:text-5xl font-bold mb-2 text-accent">
                {timeLeft.minutes}
              </div>
              <div className="text-sm md:text-base opacity-80">Minutes</div>
            </div>
            
            <div className="bg-white bg-opacity-20 rounded-2xl p-6 backdrop-blur-sm border border-white border-opacity-30">
              <div className="text-3xl md:text-5xl font-bold mb-2 text-accent">
                {timeLeft.seconds}
              </div>
              <div className="text-sm md:text-base opacity-80">Secondes</div>
            </div>
          </div>
          
          {/* Message d'attente */}
          <div className="bg-white bg-opacity-10 rounded-2xl p-6 backdrop-blur-sm border border-white border-opacity-20 max-w-2xl mx-auto">
            <p className="text-lg md:text-xl mb-4">
              🎊 Préparez-vous à gagner des cadeaux incroyables !
            </p>
            <p className="opacity-80">
              Participation 100% gratuite • Tirage au sort hebdomadaire • Cadeaux exclusifs
            </p>
          </div>

          {/* Message spécial pour la page admin */}
          {isAdminPage && (
            <div className="mt-8 bg-red-500 bg-opacity-20 rounded-2xl p-6 border border-red-400">
              <p className="text-lg font-semibold text-red-200">
                ⚠️ L'administration n'est accessible qu'après le lancement
              </p>
            </div>
          )}
        </div>
      </div>
    );
  }

  // Écran de chargement
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-primary via-secondary to-accent flex items-center justify-center">
        <div className="text-center text-white">
          <div className="animate-spin rounded-full h-16 w-16 border-b-2 border-white mx-auto mb-4"></div>
          <p className="text-xl">Chargement...</p>
        </div>
      </div>
    );
  }

  // Application normale après le lancement
  return (
    <div className="App font-main">
      {/* Ne pas afficher le Header sur la page admin */}
      {!isAdminPage && <Header />}
      
      {/* Afficher le composant correspondant à la route */}
      {isAdminPage ? <Admin /> : (
        <>
          <Home />
          <About />
        </>
      )}
    </div>
  );
}

export default App;